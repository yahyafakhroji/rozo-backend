/**
 * Transfers Function
 * Handles wallet transfers including EVM and Stellar operations
 * Migrated from /wallets/:walletId to /transfers/*
 */

import { Hono } from "jsr:@hono/hono";
import { cors } from "jsr:@hono/hono/cors";
import { Buffer } from "node:buffer";
import { encodeFunctionData, erc20Abi } from "npm:viem";
...
// Privy utilities - centralized client and wallet operations
import {
  getPrivyClient,
  getWallet,
  PrivyClient,
...
const app = new Hono().basePath("/transfers");

// ============================================================================
// Types
// ============================================================================

// Use PrivyWallet from privy.utils.ts instead of local WalletResponse
...
async function checkIfWalletHasOwner(walletId: string): Promise<PrivyWallet> {
  debugLog("Checking wallet owner", { walletId });

  const result = await getWallet(walletId);

  if (!result.success || !result.wallet) {
    debugError("Failed to fetch wallet owner", result.error);
    throw new Error(`Failed to fetch wallet owner: ${result.error}`);
  }

  debugSuccess("Wallet owner retrieved", { ownerId: result.wallet.owner_id });
...
async function validateTransactionRequest(
  ...
  walletId: string,
): Promise<{ token: string; walletOwner: PrivyWallet }> {
...
/**
 * POST /transfers/evm - EVM Transfer (USDC on Base)
 * PIN validation is handled by pinValidationMiddleware
 */
app.post("/evm", pinValidationMiddleware, async (c) => {
  debugLog("Starting EVM transaction process");
  ...
  const walletId = transactionRequest.walletId;
...
/**
 * POST /transfers/stellar/trustline - Enable USDC trustline on Stellar
 * PIN validation is handled by pinValidationMiddleware
 */
app.post("/stellar/trustline", pinValidationMiddleware, async (c) => {
  debugLog("Enable USDC trustline - start");

  // Parse request body for walletId
  const body = await c.req.json().catch(() => ({}));
  const walletId = body.walletId;

  if (!walletId) {
    return c.json({ success: false, error: "walletId is required" }, 400);
  }

  debugLog("Enable USDC - walletId", { walletId });
...
/**
 * POST /transfers/stellar - Transfer USDC on Stellar
 * PIN validation is handled by pinValidationMiddleware
 */
app.post("/stellar", pinValidationMiddleware, async (c) => {
  debugLog("Stellar Transfer - start");
  ...
  const walletId = transferRequest.walletId;

  debugLog("Stellar Transfer - walletId", { walletId });
